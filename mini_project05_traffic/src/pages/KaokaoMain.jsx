  ...
  const drawLine = (data) => {
    ...
    const validLinks = linkGeoJson.features.filter((link) => {
      return Object.values(data).some(
        (p) => p.linkId === link.properties.link_id
      );
    });
    // console.log("유효 링크값",validLinks);
    validLinks.map((el) => {
      if (
        Object.values(data).some((p) => {
    ...
    <>
      ...
      <article className={styles.main}>
        ...
        >
          ...
          {openedRoute &&
            selectedRouteList &&
            selectedRouteList.map((item) => {
              return (
                item.bsNm != selectedStop?.bsNm && (
                  <MapMarker
              ...
              >
                ...
                {arrivalInfo?.list?.length > 0 ? (
                  arrivalInfo.list.map((item) => (
                    <div
                      className={
                        item.routeNo === selectedRoute?.routeNo &&
                        styles.selectedBus
                      }
                      style={{
