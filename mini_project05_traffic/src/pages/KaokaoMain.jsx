...
function KaokaoMain(props) {
  ...
  const [orginRoute, setOriginRoute] = useState(null);
  ...
  useEffect(() => {
    fetch("/link_20250224.json").then(res => res.json()).then(res => {
      setLinkGeoJson(res);
    });
      ...
      <>
        ...
        <article className={styles.main}>
          ...
          >
            ...
            {openFind && orginRoute && (
                  <MapMarker
                      key={`findRoute_${orginRoute.lat}_${orginRoute.lng}`}
                      position={{lat: orginRoute.lat, lng: orginRoute.lng}}
                      image={{
                        src: "/stop_marker.png",
                        size: {
                          width: 50,
