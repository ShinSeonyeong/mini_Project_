...
const BoardManage = () => {
  ...
  const handleDelete = async (posts) => {
    if (!posts || posts.length === 0) {
      message.warning({content: '삭제할 게시글을 선택해주세요.', key, duration: 2});
      return;
    }
  ...
  return (
      <div className={styles.content}>
        ...
        <Modal
            title={<div style={{paddingLeft: '20px', fontSize: '23px'}}>{isEditMode ? '게시글 수정' : '게시글 등록'}</div>}
            open={isModalOpen}
            onCancel={() => {
